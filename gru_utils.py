...
def print_sentence(s, index_to_word):
    if s is None or index_to_word is None: 
        print ("warning: s or index_to_word is none",type(s),type(index_to_word))
        return ""
    sentence_str = [index_to_word[x] for x in s[1:-1]]
    sentence = " ".join(sentence_str)
    print(sentence)
    sys.stdout.flush()
    return sentence
...
def generate_sentence(model, index_to_word, word_to_index, min_length=5):
    ...
    while not new_sentence[-1] == word_to_index[SENTENCE_END_TOKEN]:
        ...
        if len(new_sentence) > 800 : #modified by Han
            return [SENTENCE_START_TOKEN,SENTENCE_END_TOKEN]
